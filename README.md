...
- ⏩ **左右水平滑动快进 / 快退与实时视频帧缩略图预览**：
  - 向左滑动快退，向右滑动快进。
  - 视频窗口顶部栏弹出**超高透极简毛玻璃 HUD 胶囊浮层**，展开当前目标时间点的**高清视频帧截图预览窗口**（基于 B 站官方 Videoshot 雪碧图引擎，零额外视频流开销，60 FPS 顺滑跟手）。
  - 实时显示进度跳转差值（如 `+15s` / `-20s`）、目标时间戳与进度条，松手精准跳转，丝毫不遮挡视频正中画面。
  - 支持灵敏度跨度调节（30s ~ 240s），设置面板支持一键开启/关闭缩略图预览。
...
| **水平向右划动** | 视频任意区域 | **视频快进 (Seek Forward)** | ⏩ **顶栏高清视频帧截图**、高透快进 `+Xs`、目标时间戳与进度条 |
| **水平向左划动** | 视频任意区域 | **视频快退 (Seek Backward)** | ⏪ **顶栏高清视频帧截图**、高透快退 `-Xs`、目标时间戳与进度条 |
