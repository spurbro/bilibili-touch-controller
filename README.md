...
- 👆 **智能双击与触控优化**：
  - 双击画面中央：切换播放 / 暂停。
  - 双击画面两侧：快速向前 / 向后跳过 10 秒（跳过片头片尾神器）。
  - 单击唤起/隐藏控制栏。
- 🛡️ **智能防误触与幽灵点击拦截**：
...
| **双击屏幕中央** | 视频中心区域 | 播放 / 暂停切换 | ▶ 播放 / ⏸ 暂停 状态图标 |
| **双击屏幕两侧** | 左侧 / 右侧边缘 | 快退 10 秒 / 快进 10 秒 | ⏪ `-10s` / ⏩ `+10s` 波纹动画 |
...
<div align="center">
  <img src="popup/popup.png" alt="Popup UI Preview" width="340" style="border-radius: 12px; box-shadow: 0 8px 24px rgba(0,0,0,0.3);" />
</div>

- **左右滑动快进/快退开关**：可单独开启或关闭，并可调节滑动跨度（30秒 ~ 240秒）。
- **音量与亮度手势独立开关**：按需启用垂直滑动手势。
- **长按倍速档位选择**：支持 2.0x / 2.5x / 3.0x / 4.0x。
- **双击响应模式**：智能模式（两侧跳跃10s，中间暂停）、仅播放暂停、仅10s跳跃。
